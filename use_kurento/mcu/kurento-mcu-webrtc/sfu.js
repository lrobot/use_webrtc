...
import expweb from './expweb'
import { Session, Register } from './lib';

let userRegister = new Register();
let rooms = {};

let io = socketIO(expweb.server).path('/groupcall');
let wsUrl = url.parse(expweb.ws_uri).href;
...
function join(socket, room, userName, callback) {
    ...
    room.pipeline.create('WebRtcEndpoint', (error, outgoingMedia) => {
        ...
        // ICE 
        // listener
        userSession.outgoingMedia.on('IceCandidateFound', event => {
            // ka ka ka ka ka
